...
class DWBaseConfig:
    ...
    def return_defaults(self, section, key):
        ...
        print(
            "Key {} not found in section {}: using default value {}".format(
...
class DWutils:
    ...
    def calc_mbwi(bands, factor, mask):
        ...
        min_cte = np.min(
            [
                np.min(bands["Green"][~mask]),
                np.min(bands["Red"][~mask]),
                np.min(bands["Nir"][~mask]),
                np.min(bands["Mir"][~mask]),
                np.min(bands["Mir2"][~mask]),
            ]
        )
        if min_cte <= 0:
            min_cte = -min_cte + 0.001
        ...
        mbwi[~mask] = (
            ...
            .fit_transform(mbwi[~mask].reshape(-1, 1))
            .reshape(-1)
        )
        mbwi[~mask] = (
            MinMaxScaler(feature_range=(-1, 1), copy=False)
            .fit_transform(mbwi[~mask].reshape(-1, 1))
            .reshape(-1)
        )
    ...
    def rgb_burn_in(
        ...
        if color:
            new_red = np.where(
                burn_in_array == valid_value,
                color[0] * (1 - transp) + red * (transp),
                red * fade,
            )
            new_green = np.where(
                burn_in_array == valid_value,
                color[1] * (1 - transp) + green * (transp),
                green * fade,
            )
            new_blue = np.where(
                burn_in_array == valid_value,
                color[2] * (1 - transp) + blue * (transp),
                blue * fade,
            )
            ...
            rgb_burn_in_values = DWutils.gray2color_ramp(
                burn_in_values,
                min_value=min_value,
                max_value=max_value,
                colormap=colormap,
            ...
            new_blue = np.where(
                burn_in_array == no_data_value, blue * fade, burn_in_blue
            )
    ...
    @staticmethod
    def array2raster(
        filename, array, geo_transform, projection, nodatavalue=0, dtype=None
    ):
    ...
    @staticmethod
    def array2rgb_raster(
        filename, red, green, blue, geo_transform, projection, nodatavalue=-9999
    ):
    ...
    def tif_2_pdf(tif_file, resolution=600, scale=2000):
        ...
        translate = (
            f"gdal_translate -outsize {resolution} 0 -ot Byte -scale 0 {scale} -of pdf "
            + tif_file
            + " "
            + pdf_file
        )
    ...
    def plot_graphs(
        ...
        for bands_names in graphs_bands:
            ...
            graph_options = {
                "title": graph_title + ":" + bands_names[0] + "x" + bands_names[1],
                "x_label": bands_names[0],
                "y_label": bands_names[1],
            }

            cluster_names = {
                1: {"name": "Water", "color": "deepskyblue"},
            ...
            data = np.c_[
                ...
                y_values[~invalid_mask],
                labels_array[~invalid_mask],
            ]
    ...
    def create_composite(bands, folder_name, pdf=True, resolution=600, offset=0):
        ...
        if pdf:
            cmd = (
                f"gdal_translate -of pdf -ot Byte -scale {v_min} {v_max} -outsize {resolution} 0 "
                + composite_base_name
                + ".vrt "
                + composite_base_name
                + ".pdf"
    ...
    @staticmethod
    def create_colorbar_pdf(
        product_name, title, label, colormap, min_value, max_value, log_scale=False
    ):
    ...
    def read_gdal_ds(file, shape_file, temp_dir):
        ...
        if gdal_mask and shape_file:
            ...
            clipped_mask_ds = gdal.Warp(
                destNameOrDestDS=dest_name, srcDSOrSrcDSTab=gdal_mask, options=opt
            )
    ...
    def extract_angles_from_xml(xml):
        ...
        for i in range(len(zenith_angle)):
            ...
            Tetag = np.cos(viewA) * np.cos(SZA) - np.sin(viewA) * np.sin(SZA) * np.cos(
                phi
            )
    ...
    @staticmethod
    def write_pdf(
        filename, text, size=(300, 50), position=(5, 5), font_color=(0, 0, 0)
    ):
        out = Image.new("RGB", size, (255, 255, 255))
        draw = ImageDraw.Draw(out)

        draw.multiline_text(
            position, text, fill=font_color, anchor=None, spacing=10, align="center"
        )
